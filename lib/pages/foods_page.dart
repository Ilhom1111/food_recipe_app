...
class FoodsPage extends StatefulWidget {
  ...
  const FoodsPage({super.key, required this.categorie});
...
class _FoodsPageState extends State<FoodsPage> {
  CategorieFoods? categorieFoods;
  @override
  void initState() {
    context
        .read<CategorieFoodsBloc>()
        .add(CategorieFoodsParsEvent(widget.categorie));
    super.initState();
  }
        ...
        listeners: [
          BlocListener<CategorieFoodsBloc, CategorieFoodsState>(
            listener: (context, state) async {
              ...
              if (state is CreateCategorieFoodsSuccess) {
                categorieFoods = state.categorieFoods;
              }
            },
          ),
        ],
        child: BlocBuilder<CategorieFoodsBloc, CategorieFoodsState>(
          builder: (context, state) {
            if (state is CategorieFoodsLoading) {
              ...
              if (categorieFoods != null) {
                return AnimationLimiter(
                  child: Padding(
                    padding:
                        EdgeInsets.only(left: 10.w, right: 10.w, top: 10.h),
                    child: GridView.builder(
                      itemCount: categorieFoods!.meals.length,
                      ...
                      itemBuilder: (context, index) {
                        final food = categorieFoods!.meals[index];
